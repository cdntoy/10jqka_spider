...
- **命令行板块选择**: 新增 `-B/--boards` 参数，支持数字编号选择板块
  - 板块编号: 1=同花顺行业 2=概念 3=地域
  - 示例: `python3 main.py -u 用户名 -p 密码 -s -B 2`（只抓概念）
  - 支持多选: `-B 2 3`（抓取概念+地域）
  - 覆盖配置文件中的 `enabled_boards` 设置

- **启动时显示板块提示**: 程序启动时显示板块类型说明和本次抓取的板块

### 改进

- 使用单字母参数 `-B` 代替长参数，更简洁
- 使用数字1/2/3代替中文名称，避免输入问题
- 更新帮助信息，添加板块编号说明
- 优化README.md文档，补充板块编号映射表
