...
1. **Socket代理模式（`-s`参数，推荐）**：通过本地socket代理转发到百度CDN
   - ✅ **当前可用**
   - 出口IP: 180.101.81.x（百度CDN IP段）
   - 需先启动socket代理：`./socket/thread_socket -r 110.242.70.68 -p 8080 -d`
   - 编译时已禁用stack protector（`-fno-stack-protector`）
   - 修复了3个buffer overflow bug（LEN_URL_STR、snprintf大小、偏移计算）

2. **本地直连模式（`-d`参数）**：直接使用本地IP访问同花顺
   - ✅ **当前可用**
   - 适用于服务器IP未被封禁的场景
   - 仅限测试和小规模使用

3. **CDN适配器模式（已弃用）**：
   - ❌ **已被封禁** - 百度CDN IP段被同花顺ATS加入黑名单
   - 已从代码中移除cdn_adapter.py

## 1. 整体架构

```
┌─────────────┐     ┌───────────────┐     ┌────────────────┐     ┌──────────────┐
│   main.py   │────▶│thread_socket  │────▶│  百度CDN节点   │────▶│  同花顺服务器 │
│  爬虫主程序  │     │  Socket代理   │     │ 110.242.70.68  │     │  q.10jqka.cn │
│ (-s模式)     │     │ (CONNECT隧道) │     │                │     │              │
└─────────────┘     └───────────────┘     └────────────────┘     └──────────────┘
       │                                          │
       │                                          ▼
